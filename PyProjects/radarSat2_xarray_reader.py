...
def get_dic_orbit_information(dictio):
    ...
    xPosition = {
        ...
        "attr": {
            "units": "",
            "xpath": os.path.join(xpath_dict['geolocation_grid']['xpath'], "stateVector", "xPosition")
        }
    }
    yPosition = {
        "values": [],
        "attr": {
            "units": "",
            "xpath": os.path.join(xpath_dict['geolocation_grid']['xpath'], "stateVector", "yPosition")
        }
    }
    zPosition = {
        "values": [],
        "attr": {
            "units": "",
            "xpath": os.path.join(xpath_dict['geolocation_grid']['xpath'], "stateVector", "zPosition")
        }
    }
    xVelocity = {
        "values": [],
        "attr": {
            "units": "",
            "xpath": os.path.join(xpath_dict['geolocation_grid']['xpath'], "stateVector", "xVelocity")
        }
    }
    yVelocity = {
        "values": [],
        "attr": {
            "units": "",
            "xpath": os.path.join(xpath_dict['geolocation_grid']['xpath'], "stateVector", "yVelocity")
        }
    }
    zVelocity = {
        "values": [],
...
def get_dic_attitude_info(dictio):
    ...
    yaw = {
        ...
        "attr": {
            "units": "",
            "xpath": os.path.join(xpath_dict['attitude_information']['xpath'], "attitudeAngles", "yaw")
        }
    }
    roll = {
        "values": [],
        "attr": {
            "units": "",
            "xpath": os.path.join(xpath_dict['attitude_information']['xpath'], "attitudeAngles", "roll")
        }
    }
    pitch = {
        "values": [],
    ...
    for key in content_dict:
        ...
        if key == "dopplerCentroid":
            ...
            for value in content_dict[key]:
                ...
                AmbiguityConfidence["attr"]["xpath"] = os.path.join(xpath, "dopplerAmbiguityConfidence")
                ...
                CentroidReferenceTime["attr"]["xpath"] = os.path.join(xpath, "dopplerCentroidReferenceTime")
                ...
                CentroidPolynomialPeriod["attr"]["xpath"] = os.path.join(xpath, "dopplerCentroidPolynomialPeriod")
                CentroidCoefficients["values"].append(
                    [float(x) for x in value["dopplerCentroidCoefficients"].split(" ")])
                CentroidCoefficients["attr"]["xpath"] = os.path.join(xpath, "dopplerCentroidCoefficients")
                CentroidConfidence["values"].append(float(value["dopplerCentroidConfidence"]))
                CentroidConfidence["attr"]["xpath"] = os.path.join(xpath, "dopplerCentroidConfidence")
        elif len(times) != 0:
...
def create_dataset_doppler_centroid(ds_attr, times, Ambiguity, AmbiguityConfidence, CentroidReferenceTime,
    ...
    ds.attrs = (ds_attr | generate_doc_ds(os.path.join(xpath_dict['doppler']['xpath'], "dopplerCentroid"), folder_path))
    return ds
...
def create_dataset_doppler_rate_values(ds_attr, rateTime, rateCoefficients, folder_path):
    ...
    ds.attrs = (ds_attr |
                generate_doc_ds(os.path.join(xpath_dict['doppler']['xpath'], "dopplerRateValues"), folder_path))
    return ds
...
def create_dataset_chirp(pole, ds_attr, replicaQualityValid, crossCorrelationWidth, sideLobeLevel,
    ...
    """
    ds = xr.Dataset()
    ds.attrs = (ds_attr | generate_doc_ds(os.path.join(xpath_dict['doppler']['xpath'], "chirp"), folder_path))
            ...
            for value in content_dict[key]:
                ...
                for k in value:
                    if "@" in k:
                        ...
                        principal_dic[var_name]["attr"]["xpath"] = os.path.join(prefix_path, k)
                        for intern_key in value[k]:
                            if "@" in intern_key:
...
def list_xsd_files(xpath, folder_path):
    ...
    interesting_files = [[file for file in list_files if (var_name in os.path.basename(file))],
                         [file for file in list_files if (parent_var_name in os.path.basename(file))],
                         [file for file in list_files if (grand_parent_var_name in os.path.basename(file))]]
